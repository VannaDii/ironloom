...
Validation coverage now asserts that the deep-test before-cleanup hook runs before container removal, that the live-lab probe persists the Gateway-bound thread session before exposing controls, that the probe creates and binds a real implementation thread, that the probe routes one returned button component id, that button-route failures fail the lab, that the optional hold executes before runtime cleanup, and that the live-lab documentation describes the manual operator acceptance path.
