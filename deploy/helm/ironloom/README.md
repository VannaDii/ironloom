...
`IRONLOOM_CONFIG_KEY` and `IRONLOOM_INSTALLER_TOKEN` are expected from the `setup.existingSecret` secret. Other runtime credentials are optional secret bindings so the pod can start and show the setup page before all credentials exist. Environment values take precedence over encrypted local setup values. OpenAI authentication can be bound through `openai.apiKeyKey` or `openai.oauthSessionKey`.
