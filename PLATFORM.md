...
- `@vannadii/devplat-config`: current code covers normalized runtime config for GitHub API/web/token settings, storage directories, worktree sync defaults, Docker/Helm deployment defaults, Discord Gateway interaction transport, OpenClaw, Sonar, shared repository/branch/timestamp validation, and structured validation issues.
- `@vannadii/devplat-artifacts`: current code covers artifact envelopes, the default lifecycle artifact registry, explicit migration records, ordered migration-path lookup, approval, audit, merge, rebase, active-registry-aware validation with migration-id/path diagnostics, and registered research/spec/slice/task/review handoff contracts.
- `@vannadii/devplat-memory`: current code covers memory-entry persistence plus decision-log, known-trap, reusable context-bundle modeling, and shared timestamp validation for durable memory records.
- `@vannadii/devplat-research`: current code covers structured research briefs, capability comparisons, feasibility structure, source attribution, and shared timestamp validation for research records.
- `@vannadii/devplat-specs`: current code covers spec records, approval, explicit revision metadata, source artifact references, shared timestamp validation for spec history, and PR-ready spec rendering with metadata, criteria, sources, and revision history.
- `@vannadii/devplat-slicing`: current code covers slice plans, readiness checks, explicit dependency-graph artifacts, PR-sized work packets with task counts, PR title hints, review focus, shared timestamp validation for slice records/graphs, and shared Git branch validation for work-packet refs.
- `@vannadii/devplat-queue`: current code covers task creation, claim, lifecycle updates, release/resume transitions, explicit transition-event history, and shared timestamp validation for durable task records and transition events.
- `@vannadii/devplat-worktrees`: current code covers pure allocation/sync/release records, branch safety checks with fail-closed Git execution, Git-backed worktree add, fetch/rebase or fast-forward sync, archive/delete release methods, shared timestamp validation on persisted worktree records, shared Git branch validation for sync base refs, fail-closed sync blocking before unsafe base refs reach Git, and preservation of failed Git command exit codes and captured streams.
- `@vannadii/devplat-execution`: current code covers structured subprocess execution, service-level repository-relative cwd refusal, timeouts, caller-configured retryable exit-code policy, truncation policy, and retry outcome contracts.
- `@vannadii/devplat-gates`: current code covers gate execution, reports, failure classification, remediation handoff hooks, next-action hints, and shared timestamp validation for gate reports/hooks.
- `@vannadii/devplat-sonarcloud`: current code covers bootstrap verification and quality gate interpretation with shared timestamp validation, normalized issue records, and Sonar issue projection into review findings for remediation planning.
- `@vannadii/devplat-review`: current code covers structured review findings, review summary generation, spec-vs-implementation conformance contracts, and shared timestamp validation for durable review records.
- `@vannadii/devplat-remediation`: current code covers remediation planning from review findings and gate remediation hooks, remediation result records, unresolved-issue summaries, and shared timestamp validation for remediation records.
...
- `@vannadii/devplat-supervisor`: current code covers next-step decisions, telemetry, lifecycle signals, blocker-aware route plans, phase routing across research, specs, slicing, implementation, gates, review, remediation, merge, continuation, headless continuation decisions from repository/objective/artifact state to the next concrete platform tool, and shared timestamp validation for supervisor and continuation decisions.
- `@vannadii/devplat-observability`: current code covers telemetry events, audit records, run metrics, run summaries, persisted audit/telemetry evidence, and shared timestamp validation for persisted observability records.
- `@vannadii/devplat-github`: current code covers GitHub action requests, policy-aware submission semantics, concrete REST request submission for PR create/update/comment/merge and branch sync, non-2xx REST receipt classification as unsubmitted decisions, normalized repository state with shared branch validation, normalized pull request state with shared head/base branch validation, and issue/spec/PR link contracts.
- `@vannadii/devplat-openclaw`: current code covers deterministic plugin config with shared timestamp validation, broad tool validation, adapter delegation, a single tool inventory factory used by plugin registration, task lifecycle tools that can preserve durable queue transition history from the current stored record, lifecycle policy evaluation output with action category, risk, escalation target, audit reason, privilege, and next-action metadata, storage/memory/telemetry/Discord lifecycle/GitHub submission/pull-request submission/supervisor-step/headless-continuation tools that honor the whitespace-normalized `DEVPLAT_STORAGE_ROOT`, command execution tool input that reuses the execution-owned cwd/env/timeout/truncation/retry option contract including caller-configured retryable exit codes and returns the auditable policy snapshot plus persisted telemetry event id, gate execution telemetry that records passed/failed classification and next-action metadata through the configured storage root, Sonar quality-gate telemetry that records evaluated project, coverage, blocking issue, actor, status, and next-action details through that same store, pull-request submission tools that target the whitespace-normalized configured `GITHUB_OWNER`/`GITHUB_REPO` repository identity, worktree allocation, sync, release, and dependent rebase tools that honor the whitespace-normalized configured `DEVPLAT_WORKTREE_ROOT`, optional Git-backed worktree materialization through explicit `applyToDisk` tool input, explicit base-branch requirements for Git-backed allocation, dependent rebase execution output that preserves delegated branch conflict classification, Discord control handling for both normalized control requests and operator interaction callbacks, artifact validation output that preserves structured diagnostics for migration-required failures, object-shaped tool responses with adapter-level `operationalResult` summaries for status, artifact id, persisted record key, policy decision id, telemetry event id, and next-action evidence, and hermetic deep-test coverage of durable queue transitions, headless continuation, plus callback-shaped Discord interactions through the loopback response transport; remaining gap is deepening delegated platform behavior as package behavior becomes more concrete.
- `@vannadii/devplat-discord`: current code covers thread-aware bindings, approvals, expanded operator actions, explicit `pull-request` thread sessions, shared timestamp validation for durable Discord records, dedicated `discord-thread-session` artifact envelopes for persisted thread bindings, codec-backed slash command contracts, raw Discord callback normalization, outbound Discord Gateway `INTERACTION_CREATE` routing with identify and heartbeat support, storage-backed bound-thread resolution for private runtimes, Helm values that start the private Gateway worker without a public webhook host, signature-verified interaction webhook helpers with structured component-bearing interaction responses for explicit inbound deployments, Discord ping responses, live-lab guild command registration, slash/button interaction routing, fail-closed thread ambiguity handling, typed bound work-item projection from Discord thread sessions, deferred interaction acknowledgement before state/telemetry/audit persistence, minimal deferred-completion follow-ups after bound-thread posts and after bound-thread post failures, single-pass normalization for interaction-originated route traces, failed-closed initial acknowledgement rejection and acknowledgement transport failure reporting through `responsePostError`, route-refusal acknowledgement rejection reporting through the same fail-closed diagnostic path, post-acknowledgement thread-post failure reporting through `threadPostError` for thrown and non-2xx responses, deferred-completion rejection reporting through `completionPostError`, REST response posting that names the resolved work item, live-lab status payloads with compact state/scope/item content and no stale interactive buttons, and a live-lab interaction probe that routes a callback-shaped operator command through the Discord response path while failing if callback/thread/completion receipts are missing, the bound thread is wrong, actionable button components are dropped from the posted control-plane payloads, or component custom ids and message receipt ids are not recorded; remaining gap is human operator-triggered slash/button invocation in the sandbox guild because Discord does not provide a supported bot API for clicking buttons as a user.
- `@vannadii/devplat-policy`: current code covers privileged-action decisions, explicit approval requirements, lifecycle action categories for merge, command execution, worktree release, rebase, publish, autofix, and destructive cleanup, risk levels, escalation targets, next-action hints, audit reasons, and shared timestamp validation for policy records.
...
- headless lifecycle continuation from repository/objective/artifact state to the next platform tool
...
Prioritize an end-to-end headless software-building path before broadening deeper package behavior:

1. an agent calls `continue_lifecycle` with repository, objective, actor, and known artifact state
2. the supervisor returns the next concrete platform tool and required inputs
3. OpenClaw routes into the delegated DevPlat tool surface
4. the called tool produces or updates an auditable lifecycle artifact
5. the agent calls `continue_lifecycle` again with the updated artifact state
6. human approval is requested only at explicit blockers such as unapproved specs or merge policy

That slice proves the core architecture: repository-scoped continuation, adapter correctness, artifact handling, and software-building progress without Discord state. Discord remains the operator projection path for teams that want in-thread controls.

Automated live-lab acceptance verifies the Discord interactive UX response path
directly. Human-triggered client clicks in the sandbox guild are deferred to a
manual acceptance window because Discord does not expose a supported bot API for
clicking buttons as a user.
