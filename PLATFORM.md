...
- `@vannadii/devplat-discord`: current code covers thread-aware bindings, approvals, expanded operator actions, explicit `pull-request` thread sessions, codec-backed slash command contracts, raw Discord callback normalization, live-lab guild command registration, slash/button interaction routing, fail-closed thread ambiguity handling, typed bound work-item projection from Discord thread sessions, REST response posting that names the resolved work item, and a live-lab interaction probe that routes a callback-shaped operator command through the Discord response path while failing if callback/thread receipts are missing or the bound thread is wrong; remaining gap is real user-driven slash/button invocation in the sandbox guild.
