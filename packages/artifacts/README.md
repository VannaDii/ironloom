...
Artifact validation first checks the shared supported artifact vocabulary from `@vannadii/devplat-core`, then dispatches locally owned payload codecs for approval, audit, merge-decision, and rebase-result records. Registry-supported lifecycle artifacts owned by downstream packages validate at the generic envelope boundary, while unsupported artifact types fail before generic normalization so unknown payload families cannot be persisted as valid handoffs.
