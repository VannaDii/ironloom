...
tool execution. OpenClaw storage, telemetry, and memory tools also honor the
trimmed configured storage root, so tool-driven artifacts and Discord
interaction state stay in the same repository-scoped store. Valid operator
